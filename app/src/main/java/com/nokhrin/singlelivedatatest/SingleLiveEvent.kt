...
import com.nokhrin.singlelivedatatest.RfLogger
import java.util.*
...
class SingleLiveEvent<T> : MediatorLiveData<T>() {
    ...
    override fun observeForever(observer: Observer<in T>) {
        RfLogger.d("DmNokhrin", "SingleLiveEvent::observeForever: ")//DmNokhrin
    ...
    override fun removeObserver(observer: Observer<in T>) {
        ...
        if (hasObservers()) {
            RfLogger.d(
                    TAG,
                    "removeObserver: errrrrrr Has Observers = ${hasObservers()} $observer"
            )
        } else {
            RfLogger.d(
                    TAG,
                    "removeObserverOK: observer $observer"
            )
        }
    }
